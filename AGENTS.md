...
- **One generic verify workflow.** `.github/workflows/ci.yml` is the only PR/push verification workflow (formatting, lint, typecheck, build, a11y). It is split into two mutually exclusive jobs: `verify-affected` on pull requests (`pnpm exec nx affected -t <task>`, keeping MR feedback lean) and `verify-all` on pushes to main (`pnpm exec nx run-many -t <task>`, a full safety net after merge; no SHAs needed since nothing is diffed). Both jobs must stay project-agnostic: workspace-level checks run at the root, everything app-specific runs via the affected/run-many Nx commands. Neither may hardcode project names or app paths. Adding a new app requires zero edits to `ci.yml`.
...
- **Triggers stay narrow.** App workflows use `paths:` filters covering the app directory, its inputs (e.g. scripts/actions), and the workflow file itself, plus `workflow_dispatch` for manual runs. Commit-pushing workflows must guard against self-triggering: end bot commit messages with `[skip ci]` (GitHub skips workflows for such pushes) and keep a `contains(head_commit.message, '[skip ci]')` `if:` guard as defense-in-depth for multi-commit pushes (see `jairusjoer-com-open-graph.yml`).
- **Caching:** Nx's local task cache covers task outputs via `targetDefaults` in `nx.json` (`build` and `typecheck` are cacheable; `a11y` and `open-graph` depend on `build`; Nx Cloud stays disabled via `neverConnectToCloud`); Actions caches are only for tool binaries such as Playwright browsers, keyed on `hashFiles('pnpm-lock.yaml')`.
